...
### Publications
   * 阅读时间：2023.7.16
   * 点评：这篇文章根据Transformers的设计特点来改进CNN的架构提高模型的鲁棒性，改进主要分为如下三点：1.下采样策略：将输入划分为p*p个不重叠的Patch，即kernel-size个stride都为p;2.增加深度卷积层的kernel-size模拟Self-Attention块的行为;3.减少激活和规范化层的数量。
